...
const { awardFirstLoginBadge } = require("../utils/badgeEngine");
...
// Login an existing user and handle badge logic
exports.login = async (req, res) => {
  ...
  try {
    ...
    // Update login metadata (used by badge checks)
    await updateLoginMeta(user.id);

    // Emit login event for real-time badge checks (e.g., Daily Login)
    eventEmitter.emit('userLoggedIn', user.id);
    console.log(`[LOGIN] Emitted 'userLoggedIn' event for user ID: ${user.id}`);

    // Award First Login badge if applicable
    if (isFirstLogin) {
      await awardFirstLoginBadge(user.id, pool);
      console.log(`[LOGIN] Awarded 'First Login' badge to user ID: ${user.id}`);
    }
